...
| `missingness_audit_major_individual.R` | Audits % missing for every column of the core ICIS-NPDES files, restricted to the major-individual panel population (from panel 06). Flags ≥25%-missing ("chronic") variables → `output/missingness_audit_major_individual_*.csv` and `output/chronic_missingness_major_individual_*.csv`. See [`missingness_audit_major_individual.md`](missingness_audit_major_individual.md). |
