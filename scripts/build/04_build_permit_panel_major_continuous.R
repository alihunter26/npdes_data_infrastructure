...
# ==============================================================================
# build_permit_panel_major_continuous.R
# ------------------------------------------------------------------------------
# PERMIT-by-YEAR panel (NPDES_ID x year), 2005-2025, of INDIVIDUAL permits that
# were MAJOR in EVERY year 2005-2025 (continuously operating) and had >=1
# enforcement action in the window.
#
#   Population : individual (NPD) permits, major (M) in every year 2005-2025
#                -> requires continuous permitting AND major status all 21 years
#   Inclusion  : >=1 enforcement action (formal or informal) 2005-2025
#   Spine      : BALANCED grid = included permits x 2005:2025
#
# CAVEAT (survivorship): requiring major every year excludes permits not present
# the full window (entrants / exiters), so this is a survival-selected sample.
# See build_permit_panel_major_entryexit.R for the entry/exit version.
#
# Major-by-year is reconstructed from ICIS_PERMITS version rows: for each year,
# the status is that of the most recent permit version effective by that year
# (carry-forward), held until the latest expiration/termination.
# ==============================================================================
...
OUT  <- file.path(CWA_ROOT, "data/processed/permit_panel_major_individual_continuous_2005_2025.csv")
...
# held through the latest expiration across versions (missing -> through YEAR_MAX)
...
# ---- 2. Qualifying permits: major in EVERY year 2005-2025 --------------------
qual <- status[, .(ok = all(!is.na(ymm) & ymm == "M")), by = id][ok == TRUE, id]
...
actions <- actions[NPDES_ID %in% qual]        # only qualifying permits
...
# ---- 4. Inclusion + BALANCED spine -------------------------------------------
included <- intersect(qual, unique(actions$NPDES_ID))
spine <- CJ(NPDES_ID = included, year = YEAR_MIN:YEAR_MAX)
...
message("=== continuous (major every year 2005-2025) ===")
message("Qualifying individual permits (major all years): ", length(qual))
message("Included (>=1 enforcement action)              : ", length(included))
message("Panel rows                                     : ", nrow(panel),
        "  (", length(included), " permits x 21 years)")
