...
# summarize_eff_violations_ny.R
# Reads NPDES_EFF_VIOLATIONS.csv from its zip in chunks, keeps only rows
# where NPDES_ID starts with "NY", then produces the standard summary sheet.
#
...
OUT_FILE   <- sprintf(
  file.path(CWA_ROOT, "output/eff_violations_ny_summary_%s.xlsx"),
  format(Sys.time(), "%Y-%m-%d_%H%M")
)
OUT_CSV    <- sprintf(
  file.path(CWA_ROOT, "output/eff_violations_ny_%s.csv"),
  format(Sys.time(), "%Y-%m-%d_%H%M")
)
...
DESCRIPTION   <- "description of effluent (DMR) violations for New York facilities (NPDES_ID starting with 'NY')"
SHEET_SUMMARY <- "One row per parameter/limit violation reported on a New York facility's DMR. Filtered from the full national NPDES_EFF_VIOLATIONS file to NPDES_IDs beginning with 'NY'. Includes the parameter, violation type, reported vs. limit value, exceedance %, and noncompliance detection/resolution dates."
...
read_ny_rows <- function(zip_path, csv_name, chunk_size) {
  ...
  repeat {
    ...
    va <- chunk[startsWith(as.character(chunk$NPDES_ID), "NY")]
    cat(sprintf("kept %s NY rows.\n", format(nrow(va), big.mark = ",")))

    if (nrow(va) > 0) ny_chunks[[length(ny_chunks) + 1]] <- va
    rm(chunk); gc()
  ...
  if (length(ny_chunks) == 0) stop("No New York rows found.")

  df <- rbindlist(ny_chunks)
  cat(sprintf("Total NY rows: %s\n\n", format(nrow(df), big.mark = ",")))
  as.data.frame(df)
}

# ── Build the summary from the filtered NY data ───────────────────────────────
...
build_summary <- function(df) {
  ...
  meta <- list(
    title     = paste0(CSV_IN_ZIP, " (New York only): ", DESCRIPTION),
    highlevel = SHEET_SUMMARY,
    summary   = sprintf(
      "Observations: %s, Distinct NY Permits: %s, Temporal Range: %s, Duplicate Rows: %s",
...
df           <- read_ny_rows(ZIP_PATH, CSV_IN_ZIP, CHUNK_SIZE)
fwrite(df, OUT_CSV)
cat("CSV saved to:", OUT_CSV, "\n")
summary_list <- build_summary(df)

wb <- createWorkbook()
write_sheet(wb, "EFF_VIOLATIONS_NY", summary_list)
