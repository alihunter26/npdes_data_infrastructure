...
numeric layout (a trailing, always-blank **Missing Explanation** column).

Two cross-tab scripts build a different kind of output (coverage matrices, not
per-variable summaries) and remain in this folder alongside `summarize.R`:
`summarize_dmr_coverage_major_minor.R` and `summarize_year_coverage.R`.
