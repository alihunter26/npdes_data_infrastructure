...
# ------------------------------------------------------------------------------
# One registry-driven script covering every dataset (npdes, dmrs, attains,
# eff_violations, eff_violations_state, limits, master_general_permits,
# outfalls_layer). The shared machinery — styles, the per-variable helpers, and
# the worksheet writer — lives here ONCE; each dataset is a config entry in the
# DATASETS registry near the bottom.
#
...
if (!interactive() && !nzchar(Sys.getenv("SUMMARIZE_NO_MAIN"))) {
  ...
  if (length(args) == 0L) {
    cat("Usage: Rscript code/summary/summarize.R <dataset> [arg]\n",
