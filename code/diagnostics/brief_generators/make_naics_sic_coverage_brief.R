...
#   itself produced by code/diagnostics/naics_sic/naics_sic_coverage_by_state_year.R
...
# End the fragment with \bottomrule (NOT a row-ending \\): a file \input into a
# tabular must not end on "\\" immediately before a parent \noalign rule -- that
# triggers "Misplaced \noalign". Ending on the rule itself is safe.
writeLines(c(state_lines, "\\midrule", total_line, "\\bottomrule"),
...
writeLines(c(year_lines, "\\bottomrule"),
           file.path(OUT_DIR, "tab_naics_sic_by_year_rows.tex"))
