...
#   N_STATE_AFR           - ...of ACTIVITY_TYPE_CODE "AFR" led by a STATE agency
#   N_EPA_AFR             - ...of ACTIVITY_TYPE_CODE "AFR" led by EPA
#   N_STATE_JDC           - ...of ACTIVITY_TYPE_CODE "JDC" led by a STATE agency
#   N_EPA_JDC             - ...of ACTIVITY_TYPE_CODE "JDC" led by EPA
...
#      total from below either. AGENCY (State/EPA) is expected to be one-per-action,
#      so N_STATE_AFR + N_EPA_AFR is expected to PARTITION N_AFR, and likewise
#      N_STATE_JDC + N_EPA_JDC to partition N_JDC. For informal, OFFICIAL_FLG (Y/N)
#      is expected to be one-per-action and thus to partition the informal total --
...
informal_counts <- informal[, .(
    ...
    N_NOV                 = sum(ENF_TYPE_CODE == "NOV"),
    N_NONC                = sum(ENF_TYPE_CODE == "NONC"),
    N_AER                 = sum(ENF_TYPE_CODE == "AER"),
    N_OFFICIAL_INFORMAL   = sum(OFFICIAL_FLG == "Y"),
    N_UNOFFICIAL_INFORMAL = sum(OFFICIAL_FLG == "N")
  ), by = .(facility_id, YEAR, MONTH)]
...
count_cols <- c("N_FORMAL_ACTIONS", "N_AFR", "N_JDC",
                ...
                "N_STATE_AFR", "N_EPA_AFR", "N_STATE_JDC", "N_EPA_JDC",
...
message("  State AFR / EPA AFR / State JDC / EPA JDC       : ",
        sum(panel$N_STATE_AFR, na.rm = TRUE), " / ", sum(panel$N_EPA_AFR, na.rm = TRUE), " / ",
        sum(panel$N_STATE_JDC, na.rm = TRUE), " / ", sum(panel$N_EPA_JDC, na.rm = TRUE))
message("  Identity state + epa == AFR / JDC totals holds  : ",
        all(panel$N_STATE_AFR + panel$N_EPA_AFR == panel$N_AFR, na.rm = TRUE), " / ",
        all(panel$N_STATE_JDC + panel$N_EPA_JDC == panel$N_JDC, na.rm = TRUE))
